...
function parseAndGenerateCSVCustom($rawData, $fileType, $targetFolder, $outputFolder) {
    ...
    // Create output directory with custom folder name
...
function cleanLocationFiles($locationPath, $requiredFileTypes, $outputFolder) {
    ...
    foreach ($csvFiles as $filePath) {
        ...
        // Clean the file
        echo "  Cleaning: $filename -> $fileType (date: $dateStr)\n";
        ...
        try {
            ...
            // Use existing parseAndGenerateCSV function to clean the data
            // But we need to modify the output directory
            parseAndGenerateCSVCustom($rawData, $fileType, $locationName, $outputFolder);
            
            $locationStats['files_processed']++;
            $stats['total_files_processed']++;
            
            echo "    Success: File cleaned and saved\n";
...
echo "Next step: Use the cleaned CSV files for further processing\n";
?>
